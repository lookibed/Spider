...
int te_debug_builtin_count(void) {
    return (int)(sizeof(functions) / sizeof(functions[0])) - 1;
}

int te_debug_builtin_type(int index) {
    if (index < 0 || index >= te_debug_builtin_count()) {
        return -1;
    }
...
int te_debug_builtin_name_char(int index, int char_index) {
    if (index < 0 || index >= te_debug_builtin_count() || char_index < 0) {
        return -1;
    }
    ...
    return (unsigned char)functions[index].name[char_index];
}
...
int te_debug_builtin_name_pointer(int index) {
    ...
    return (int)functions[index].name;
}
...
int te_debug_builtin_address_pointer(int index) {
    ...
    return (int)functions[index].address;
}

int te_debug_find_builtin_index(const char *name, int len) {
    const te_variable *found = find_builtin(name, len);
    if (!found) {
        return -1;
    }

    return (int)(found - functions);
}
...
int te_debug_compare_builtin(const char *name, int len, int index) {
    ...
    c = strncmp(name, functions[index].name, len);
    if (!c) {
        c = '\0' - functions[index].name[len];
    }
...
int te_debug_find_builtin_trace_step(const char *name, int len, int step) {
    ...
    while (imax >= imin) {
        ...
        if (iteration == step) {
            return (i << 8) | (branch << 4) | (c == 0 ? 0 : (c > 0 ? 1 : 2));
        }
